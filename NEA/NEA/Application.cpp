...
Application::Application(): window(sf::VideoMode(1920,1080), "Heavy Metal Jamboree", sf::Style::Close)
    , textures(), fonts(), player(), music(), sounds()
    , stateStack(State::Context(window, textures, fonts, player, /*OnlinePlayer,*/ music, sounds, p1Char, p2Char, backGroundFilePath, platformFilePath, hudLeftFilePath, hudRightFilePath))
    , statsText(), statsUpdateTime(), statsNumFrames(0), skicon()

{
    ...
    textures.load(TextureID::TitleScreen, "Media/Textures/Fight/rockWorldTour.png");
    ...
    textures.load(TextureID::ControllScreen, "Media/Textures/Fight/controlls.jpg");
...
void Application::updateStatistics(sf::Time et)
{
    ...
    if (statsUpdateTime >= sf::seconds(1.0f)) {
        ...
        statsUpdateTime -= sf::seconds(1.f);
        statsNumFrames = 0;
    }
}
...
void Application::registerStates()
{
    ...
    stateStack.registerState<ControllState>(StateID::Controlls);
    ...
    stateStack.registerState<InitOnlineState>(StateID::InitOnline);
    stateStack.registerState<OnlineGameState>(StateID::OnlineGame);
}