...
MapSelectionState::MapSelectionState(StateStack& stack, Context context):State(stack, context), mOptions()
	, mOptionIndex(0)
{
	sf::Font& font = context.fonts->get(FontID::Main);
	backgroundSprite.setTexture(context.textures->get(TextureID::MapSelectionScreen));
	menuTitle.setFont(font);
	menuTitle.setString("Destination Selection");
	...
	platformFilePaths.insert(std::make_pair(OptionNames::M1, "Media/Textures/Fight/texasPlatform.png"));
	platformFilePaths.insert(std::make_pair(OptionNames::M2, "Media/Textures/Fight/antarcticPlatform.png"));
	platformFilePaths.insert(std::make_pair(OptionNames::M3, "Media/Textures/Fight/britishPlatform.png"));
	platformFilePaths.insert(std::make_pair(OptionNames::M4, "Media/Textures/Fight/tokyoPlatform.png"));
	...
	if (event.key.code == sf::Keyboard::Return)
	{
		if (mOptionIndex == M1) {
			...
			*getContext().platformfilePath = platformFilePaths[M1];
			getContext().music->setMusicType(MusicPlayer::MUSIC_TYPE::Texas);
			...
			*getContext().platformfilePath = platformFilePaths[M2];
			getContext().music->setMusicType(MusicPlayer::MUSIC_TYPE::Japan);
			requestStackPop();
			...
			*getContext().platformfilePath = platformFilePaths[M3];
			getContext().music->setMusicType(MusicPlayer::MUSIC_TYPE::Sweeden);
			requestStackPop();
			requestStackPush(StateID::Game);
		}
			...
			*getContext().platformfilePath = platformFilePaths[M4];
			getContext().music->setMusicType(MusicPlayer::MUSIC_TYPE::UK);
			requestStackPop();
			requestStackPush(StateID::Game);
		}
