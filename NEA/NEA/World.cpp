...
World::World(sf::RenderTarget& outputTarget, FontHolder_t& fonts, AudioPlayer& audioQueues
	...
	: target(outputTarget), sceneTexture(), worldView(target.getDefaultView())
	, textures(), fonts(fonts)	, audioQueues(audioQueues)
	, p1Char(p1Char), p2Char(p2Char), backGroundFilePath(backGroundFilePath)
	, platformFilePath(platformFilePath), hudLeftFilePath(hudLeftFilePath)
	, hudRightFilePath(hudRightFilePath)
	, sceneGraph(), sceneLayers(), commandQueue()
	...
	, playerActor(nullptr), player2Actor(nullptr)
	, platformActor(nullptr), leftHealthBar(sf::Vector2f(400.f, 43.f))
	, rightHealthBar(sf::Vector2f(400.f, 43.f))
	, leftLight1(10.f), leftLight2(10.f), leftLight3(10.f), rightLight1(10.f)
	, rightLight2(10.f), rightLight3(10.f), isRoundOver(false)
	...
	, p1Rounds(0), p2Rounds(0), roundTimer(90)
...
void World::loadTextures()
{
	...
	textures.load(TextureID::Hero1, "Media/Textures/Fight/genre1.png");
	textures.load(TextureID::Hero2, "Media/Textures/Fight/genre2.png");
	textures.load(TextureID::Hero3, "Media/Textures/Fight/genre3.png");
...
void World::buildScene()
{
	...
	std::unique_ptr<AudioNode> audioNode(new AudioNode(audioQueues));
	sceneGraph.attachChild(std::move(audioNode));
...
{
	audioQueues.setListenerPosition(playerActor->getWorldPosition());
	audioQueues.removeStoppedAudio();
}
			...
			if (playerActor->getBoundingRect().intersects(player2Actor->getBoundingRect()))
			{
				...
				if (playerActor->getState() == Actor::State::Attack1) {
					...
					if (player2Actor->isBlocking) {
						...
						audioQueues.play(EffectID::Block);
