...
RoundOverState::RoundOverState(StateStack& stack, Context context) : State(stack, context)
	, backgroundSprite(), roundOverText(), instructionText()
{
	sf::Font& font = context.fonts->get(FontID::Main);
	sf::Vector2f viewSize = context.window->getView().getSize();
	roundOverText.setFont(font);
	roundOverText.setString("Well Done, you're not terrible");
