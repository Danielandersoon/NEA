...
ControllState::ControllState(StateStack& stack, Context context)
	:State(stack, context)
	, mOptionIndex(0)
	, mOptions()
{
	...
	backgroundSprite.setTexture(context.textures->get(TextureID::ControllScreen));
...
bool ControllState::update(sf::Time dt)
{
	return false;
}

bool ControllState::handleEvents(const sf::Event& event)
{
...
void ControllState::updateOptionText()
{
	if (mOptions.empty())
		return;
