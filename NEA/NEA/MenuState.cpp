...
MenuState::MenuState(StateStack& stack, Context context):State(stack, context), mOptions(), mOptionIndex(0)
{
	...
	sf::Text OnlineplayOption;
	OnlineplayOption.setFont(font);
	OnlineplayOption.setString("Play Online");
	centerOrigin(OnlineplayOption);
	OnlineplayOption.setPosition(playOption.getPosition() + sf::Vector2f(0.f, 50.f));
	mOptions.push_back(OnlineplayOption);

	sf::Text optionsOption;
	optionsOption.setFont(font);
	optionsOption.setString("Options");
	centerOrigin(optionsOption);
	optionsOption.setPosition(OnlineplayOption.getPosition() + sf::Vector2f(0.f, 50.f));
	mOptions.push_back(optionsOption);

	sf::Text Controlls;
	Controlls.setFont(font);
	Controlls.setString("Controlls");
	centerOrigin(Controlls);
	Controlls.setPosition(optionsOption.getPosition() + sf::Vector2f(0.f, 50.f));
	mOptions.push_back(Controlls);

	sf::Text exitOption;
	exitOption.setFont(font);
	exitOption.setString("Exit");
	centerOrigin(exitOption);
	exitOption.setPosition(Controlls.getPosition() + sf::Vector2f(0.f, 50.f));
...
bool MenuState::handleEvents(const sf::Event& event)
{
	...
	if (event.key.code == sf::Keyboard::Return)
	{
		...
		else if (mOptionIndex == OnlinePlay) {
			requestStackPop();
			requestStackPush(StateID::InitOnline);
		}
		else if (mOptionIndex == Options) {
			requestStackPop();
			requestStackPush(StateID::Options);
		}
		else if (mOptionIndex == Controlls) {
			requestStackPop();
			requestStackPush(StateID::Controlls);
		}
