...
Actor::Actor(Type type, const TextureHolder_t& textures, const FontHolder_t& fonts, Category::Type player) : Entity(100)
	, type_(type)
	, state_(State::Idle)
...
void Actor::updateMovementPattern(sf::Time dt)
{
	...
	if (Actor::Type::Hero1 == type_ || Actor::Type::Hero2 == type_ || Actor::Type::Hero3 == type_) {
		...
		if (isJumping)
		{
			...
			{
				setPosition(getPosition().x, (getPosition().y) - (gravitySpeed * 0.02 * dt.asSeconds()));
			}
			else
			{
				isJumping = false;
...
void Actor::playAudio(EffectID effectType)
{
	audioPlayer.play(effectType);
	...
	if (Actor::Type::Hero1 == type_ || Actor::Type::Hero2 == type_ || Actor::Type::Hero3 == type_) {
		...
		if (Actor::State::Dead == state_ && animations_[state_].isStarting()) {
			if (Actor::Type::Hero1 == type_)
				audioPlayer.play(EffectID::H1GetHit);
			if (Actor::Type::Hero2 == type_)
				audioPlayer.play(EffectID::H2GetHit);
			if (Actor::Type::Hero3 == type_)
				audioPlayer.play(EffectID::H3GetHit);
		}
		...
		if (Actor::State::TakeHit == state_ && animations_[state_].isStarting()) {
			if (canPlayDamageAudio) {
				playAudio(EffectID::TakeDamage);
			}

			canPlayDamageAudio = false;
		}
		...
		if (Actor::State::Attack1 == state_ && animations_[state_].isStarting()) {
			if (Actor::Type::Hero1 == type_)
				audioPlayer.play(EffectID::H1Attack1);
			if (Actor::Type::Hero2 == type_)
				audioPlayer.play(EffectID::H2Attack1);
			if (Actor::Type::Hero3 == type_)
				audioPlayer.play(EffectID::H3Attack1);
		}
		...
		if (Actor::State::Attack2 == state_ && animations_[state_].isStarting()) {
			if (Actor::Type::Hero1 == type_)
				audioPlayer.play(EffectID::H1Attack2);
			if (Actor::Type::Hero2 == type_)
				audioPlayer.play(EffectID::H2Attack2);
			if (Actor::Type::Hero3 == type_)
				audioPlayer.play(EffectID::H3Attack2);
		}
...
void Actor::updateAudios()
{
	audioPlayer.setListenerPosition(sprite_.getPosition());
	audioPlayer.removeStoppedAudio();
}
